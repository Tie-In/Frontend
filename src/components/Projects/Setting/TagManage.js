...
import {
  Col, Row, Table, Glyphicon, Button, FormGroup, FormControl, DropdownButton, MenuItem, Label, Dropdown,
                ...
                />
						...
						<Dropdown id="tagsDropdown" open={this.state.openDropdownCreate}>
							<div bsRole="toggle">
								...
								>
									<div 
										style={{ backgroundColor: this.state.newTagColor, width: 15, height: 15, display: 'inline-block' }} 
									/> {this.state.newTagColor}
								</Button>
							</div>
							<div className="dropdown-menu" style={menuStyle} bsRole="menu">
								<CirclePicker 
									onChangeComplete={(color) => { this.setState({ newTagColor: color.hex, openDropdownCreate: false }) } }
								/>
							</div>
						</Dropdown>
					</Col>
